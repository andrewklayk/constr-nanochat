...
class GPT(nn.Module):
    ...
    def calculate_unclosed_bracket_penalty(self, prob_sequence):
        ...
        # Track indices of open brackets
        open_bracket_indices = []
        open_bracket_types = []
        for pos_in_sequence, probs in enumerate(prob_sequence):
            # determine if the model predicts an open bracket at position idx
            # for now, count "argmax(probs) == token" as "predicted token"
            # smarter things are possible
            token = torch.argmax(probs).detach().cpu().item()
            if token in open_brackets:
                stack.append(token)
                open_bracket_indices.append(pos_in_sequence)
            elif token in close_brackets:
                if stack and stack[-1] == bracket_pairs[token]:
                    stack.pop()
                    open_bracket_indices.pop()

        # Penalty is the sum of probabilities of unclosed open brackets
        penalty = 0.0
        for pos_in_sequence in open_bracket_indices:
            penalty += prob_sequence[pos_in_sequence, 40]
