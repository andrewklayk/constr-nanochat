...
from nanochat import barrier as Barrier
"""
...
class PBM(Optimizer):
    def __init__(
        ...
        dual_smoothing_beta: float = 0.9, # smoothing of the dual variables
        dual_range: Tuple[float, float] = (0, 100),
        init_pi = 10.0,
        const_p = 1.0,
        epoch_len=None,
        ...
        *,
        ...
        self.dual_beta = dual_smoothing_beta
    ...
    def dual_step(self, i: int, c_val: Tensor):
        ...
        # safe-guarding 
        if self._dual_vars[i] <= self.dual_range[0]:
            self._dual_vars[i] = self.dual_range[0]
        if self._dual_vars[i] >= self.dual_range[1]:
            self._dual_vars[i] = self.dual_range[1]
