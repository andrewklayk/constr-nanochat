...
print0(torch.cuda.is_available())
...
alm_optimizer, muon_optimizer = optimizers
...
while True:
    ...
    with autocast_ctx:
        ...
        for pos in range(probs.shape[-2]-1):
            con += torch.mean(pos_impl_constraint(probs[:,pos,0], probs[:, pos+1, -13])  - 1, dim=0)
        constr_eval = [con]
    ...
    for j, cv in enumerate(constr_eval):
        ...
        alm_optimizer.dual_step(j, cv)
        alm_optimizer.zero_grad(set_to_none=True)
